...
# CONDAENV
if [ ! -n "${BULLETTRAIN_CONDAENV_SHOW+1}" ]; then
  BULLETTRAIN_CONDAENV_SHOW=true
fi
if [ ! -n "${BULLETTRAIN_CONDAENV_BG+1}" ]; then
  BULLETTRAIN_CONDAENV_BG=yellow
fi
if [ ! -n "${BULLETTRAIN_CONDAENV_FG+1}" ]; then
  BULLETTRAIN_CONDAENV_FG=white
fi
if [ ! -n "${BULLETTRAIN_CONDAENV_PREFIX+1}" ]; then
  BULLETTRAIN_CONDAENV_PREFIX=🐍
fi
...
prompt_condaenv() {
  if [[ $BULLETTRAIN_CONDAENV_SHOW == false ]]; then
    return
  fi

  local condaenv_path="$CONDA_ENV_PATH"
  if [[ -n $condaenv_path && -n $CONDA_ENV_DISABLE_PROMPT && -n $CONDA_DEFAULT_ENV ]]; then
    prompt_segment $BULLETTRAIN_CONDAENV_BG $BULLETTRAIN_CONDAENV_FG $BULLETTRAIN_CONDAENV_PREFIX" $CONDA_DEFAULT_ENV"
  fi
}
